      ...
      <div class="korpa-list">
        <div v-if="stavkeList && stavkeList.length > 0">
          <div v-for="stavka in stavkeList" :key="stavka.id" class="korpa-card">
            <div >
              Količina: {{ stavka.skrp_kolicina }}

            </div>
            <!-- Prikazivanje podataka o parfemu koristeći stavka.skr_par_id -->
            <h2 class="korpa-title">{{ parfemi[stavka.skrp_par_id]?.par_naziv }}</h2>
            <img :src="'https://localhost:44333/resources' + parfemi[stavka.skrp_par_id]?.par_slika" class="cokolada-image" alt="Logo fabrike">
            
            <p class="korpa-text">Cena po komadu: {{ stavka.skrp_cena_pj }} RSD</p>
            <p class="korpa-text">Ukupno: {{ stavka.skrp_kolicina * stavka.skrp_cena_pj }} RSD</p>
      ...
      async fetchStavkeKorpe() {
        try {
          ...
          if (korpaResponse.status === 200 && korpaResponse.data) {
            ...
            const stavkeResponse = await axios.get(`https://localhost:44333/api/stavka-korpe/korpa/${korpaId}`);
