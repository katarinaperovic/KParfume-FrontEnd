<template>
    <div class="cokolade">
      ...
      <div class="card-container">
        <div v-for="parfem in filteredParfemi" :key="parfem.id" :class="['card', { 'card-nedostupan': !parfem.par_dostupan }]">
          ...
          <div class="image-container">
            ...
            <h3 v-if="cene[parfem.id] && cene[parfem.id].sc_cena && parfem.par_dostupan"  class="cenaUgao">
           {{ cene[parfem.id].sc_cena }} rsd</h3>
       ...
       <h3 class="mililitrazaUgao">{{ parfem.par_mililitraza }}ml</h3>
          ...
          <div class="category-vrsta-container">
            ...
            {{ parfem.par_kp_id === 1 ? 'ženski' : parfem.par_kp_id === 2 ? 'muški' : parfem.par_kp_id === 3 ? 'unisex' : 'N/A' }}
          </p>
            ...
            {{ parfem.par_vp_id === 1 ? 'cvetni' : parfem.par_vp_id === 2 ? 'orijentalni' : parfem.par_vp_id === 3 ? 'citrusni' : parfem.par_vp_id === 4 ? 'drvenasti' : 'N/A' }}
          </p>
  ...
  Dodaj u korpu
      ...
      <label>Cena:</label>
      <input v-model.number="editedCena.sc_cena" required @input="validateCena" title="Molimo unesite broj >=0" />
    ...
    await Promise.all(
      parfemi.value.map(async (parfem) => {
        if (!cene.value[parfem.id]) {
          const ceneResponse = await axios.get(`https://localhost:44333/api/stavka-cenovnika/parfemId/${parfem.id}`);
...
async function saveParfem() {
  ...
  try {
    ...
    await axios.put(`https://localhost:44333/api/parfem/${editingParfem.value.id}`, editedParfem.value);

    

    // Ažuriranje cene u stavci cenovnika
    await axios.put(`https://localhost:44333/api/stavka-cenovnika/${editedCena.value.id}`, editedCena.value);
...
.nedostupan {
  ...
  background-color: rgba(255, 0, 0, 0.7); /* Poluproziran crveni pozadinski boja */
...
.cenaUgao {
  ...
  top: 10px; /* Udaljenost od vrha */
  left: 10px; /* Udaljenost od leva */
  background-color: rgba(255, 174, 0, 0.834); /* Poluproziran crveni pozadinski boja */
  color: white; /* Boja teksta */
  padding: 5px 10px;
  border-radius: 5px;
  font-size: 1.2rem;
...
.mililitrazaUgao {
  ...
  background-color: rgba(82, 8, 39, 0.834); /* Poluproziran crveni pozadinski boja */
  color: white; /* Boja teksta */
  padding: 5px 10px;
  border-radius: 5px;
  font-size: 1.2rem;
...
.vrsta-cvetni {
  background: linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url('@/assets/cvetni.jpg');
...
.vrsta-orijentalni {
  background: linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url('@/assets/orijentalni.jpg');
  background-size: cover;
  background-position: center;
}

.vrsta-citrusni{
  background: linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url('@/assets/citrusni.jpg');
  background-size: cover;
  background-position: center;
}

.vrsta-drvenasti{
  background: linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url('@/assets/drvenasti.jpg');
  background-size: cover;
  background-position: center;
}
...
.kat-zen, .kat-muski, .kat-unisex {
  font-size: 1.2rem; /* Povećanje veličine fonta */
}

.vrsta-cvetni, .vrsta-orijentalni, .vrsta-citrusni, .vrsta-drvenasti {
  font-size: 1.2rem; /* Povećanje veličine fonta */
}

.tip-zimski, .tip-jesenji, .tip-prolecni, .tip-letnji{
  font-size: 1.2rem; /* Povećanje veličine fonta */
}
